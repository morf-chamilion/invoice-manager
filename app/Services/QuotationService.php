...
class QuotationService extends BaseService
{
    ...
    /**
     * Get conversion rate from quotations to invoices.
     */
    public function getConversionRate(): float
    {
        $totalQuotations = $this->quotationRepository->getModel()
        ...
        $convertedQuotations = $this->quotationRepository->getModel()
            ->where('vendor_id', $this->getAuthVendor()?->id)
            ->where('status', '!=', QuotationStatus::DRAFT)
            ->whereNotNull('invoice_id')
            ->count();

        return round(($convertedQuotations / $totalQuotations) * 100, 2);
    }
}
