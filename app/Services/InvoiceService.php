...
class InvoiceService extends BaseService
{
     ...
     *
     * @return int $data[count]		Results Count
    ...
    /**
     * Get total amount due for all invoices.
     */
    ...
    /**
     * Get count of invoices sent this month.
     */
    public function getInvoicesSentThisMonth(): int
    {
        return $this->invoiceRepository->getModel()
            ->where('vendor_id', $this->getAuthVendor()?->id)
            ->where('status', '!=', InvoiceStatus::DRAFT)
    ...
    /**
     * Get invoice status distribution for donut chart.
     */
    ...
    public function getRevenueChartData(): array
    {
        ...
        for ($date = $startDate->copy(); $date->lte($endDate); $date->addMonth()) {
            ...
            $invoicedAmount = $this->invoiceRepository->getModel()
                ->where('vendor_id', $this->getAuthVendor()?->id)
                ->where('status', '!=', InvoiceStatus::DRAFT)
            ...
            $dueAmount = $this->invoiceRepository->getModel()
                ->where('vendor_id', $this->getAuthVendor()?->id)
                ->where('status', '!=', InvoiceStatus::DRAFT)
                ->where('payment_status', '!=', InvoicePaymentStatus::PAID)
                ->whereMonth('created_at', $date->month)
