<x-default-layout :model="$userRole">

    <form method="POST" action="{{ route(UserRoleRoutePath::UPDATE, $userRole) }}" autocomplete="off" id="resource_form">
        @method('put')
        @csrf
        ...
        <div class="d-flex flex-column flex-lg-row">
                    ...
                    <div class="card-body">
                        ...
                        <div class="mb-8">
                            ...
                            <x-input-text id="name" name="name" type="text" class="mt-1 block w-full"
                                :value="old('name', $userRole->name)" autocomplete="none" required />
                                                        ...
                                                        $selected = !empty(
                                                            array_intersect(
                                                                collect($permissions)
                                                                    ->pluck('id')
                                                                    ->toArray(),
                                                                $userRole->permissions->pluck('id')->toArray(),
                                                            )
                                                        );
                                                    @endphp
                                                    <li data-jstree='{"selected": {{ json_encode($selected) }}}'
                                                        id="@json(collect($permissions)->pluck('id'))">
                                                        {{ $action }}
                                                    </li>
                                                @endforeach
                                            </ul>
            ...
            <x-form-metadata :model="$userRole" type="Update">
                <div class="mb-10">
                    ...
                    <x-input-select name="status" data-placeholder="Select Status" data-hide-search="true" required>
                        @foreach (UserRoleStatus::toSelectOptions() as $option)
                            <option value="{{ $option->value }}" @selected($option->value == old('status', $userRole->status->value))>
                                {{ $option->name }}
                            </option>
                        @endforeach
                    </x-input-select>
