<x-default-layout>
    <form method="post" action="{{ route(PaymentRoutePath::STORE) }}" autocomplete="off" id="resource_form">
        @method('post')
        @csrf
                        ...
                        <div class="row">
                            ...
                            <div class="col-lg-6">
                                ...
                                <x-input-textarea name="notes" id="notes"
                                    style="height: 80px;">{{ old('notes') }}</x-input-textarea>
                            ...
                            <div class="col-lg-6">
                                ...
                                <x-input-file id="reference_receipt" name="reference_receipt" :fileMaxSize="2"
                                    mimeTypes="['application/pdf', 'image/*']" :value="null" />
                ...
                <div class="mb-10">
                    ...
                    <x-input-select name="status" data-placeholder="Select Status" data-hide-search="true" required>
                        @foreach (PaymentStatus::toSelectOptions() as $option)
                            <option value="{{ $option->value }}" @selected($option->value == old('status', PaymentStatus::PAID->value))>
                                {{ $option->name }}</option>
                        @endforeach
                    </x-input-select>
