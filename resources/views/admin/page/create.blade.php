<x-default-layout>
    <form method="post" action="{{ route(PageRoutePath::STORE) }}" autocomplete="off" id="resource_form">
        @method('post')
        @csrf
                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        <div class="mb-8">
                            ...
                            <x-input-text name="title" id="title" :value="old('title')" />
                            <x-input-error :messages="$errors->get('title')" />
                        </div>

                        <div class="mb-8">
                            <x-input-label for="slug" :value="__('Slug')" />
                            <x-input-text name="slug" id="slug" :value="old('slug')" />
                ...
                <div class="mb-10">
                    ...
                    <x-input-select name="status" data-placeholder="Select Status" data-hide-search="true" required>
                        @foreach (PageStatus::toSelectOptions() as $option)
                            <option value="{{ $option->value }}" @selected($option->value == old('status', PageStatus::ACTIVE->value))>
                                {{ $option->name }}
                            </option>
                        @endforeach
                    </x-input-select>
                ...
                <div class="mb-10">
                    ...
                    <x-input-select name="admin_template" data-placeholder="Select Template" data-hide-search="true"
                        ...
                        @foreach ($templates as $key => $template)
                            <option value="{{ $template['admin'] }}" @selected($template['admin'] == old('admin_template', 'admin.page.template.default'))>
                                {{ $key }}
                            </option>
                        @endforeach
                    </x-input-select>
                    <p class="text-muted mt-2">Create the form to view template changes.</p>
